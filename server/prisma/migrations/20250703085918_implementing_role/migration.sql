/*
  ...
  - You are about to drop the column `confirmed` on the `Transaction` table. All the data in the column will be lost.
  - You are about to drop the column `confirmedBy` on the `Transaction` table. All the data in the column will be lost.
  - You are about to drop the column `value` on the `Transaction` table. All the data in the column will be lost.
  - Made the column `approved_by` on table `Topup` required. This step will fail if there are existing NULL values in that column.
  - Added the required column `amount` to the `Transaction` table without a default value. This is not possible if the table is not empty.
  - Added the required column `type_id` to the `Transaction` table without a default value. This is not possible if the table is not empty.
...
CREATE TABLE "new_Topup" (
    ...
    CONSTRAINT "Topup_approved_by_fkey" FOREIGN KEY ("approved_by") REFERENCES "User" ("uid") ON DELETE RESTRICT ON UPDATE CASCADE
);
...
INSERT INTO "new_Transaction" ("createdAt", "date", "description", "trans_id", "uid", "updatedAt", "wallet_id") SELECT "createdAt", "date", "description", "trans_id", "uid", "updatedAt", "wallet_id" FROM "Transaction";
